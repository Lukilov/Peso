...
object FakeData {
    ...
    // začetne demo transakcije (prilagodi po želji)
    private val _transactions = mutableListOf(
        // PRIHODKI
        Transaction("Plača januar", BigDecimal("1200.00"), LocalDate.of(2025, 1, 15), true, "salary"),
        Transaction("Plača februar", BigDecimal("1200.00"), LocalDate.of(2025, 2, 15), true, "salary"),
        Transaction("Plača marec", BigDecimal("1200.00"), LocalDate.of(2025, 3, 15), true, "salary"),

        // STROŠKI
        Transaction("Hrana januar", BigDecimal("80.00"), LocalDate.of(2025, 1, 10), false, "food"),
        Transaction("Kava februar", BigDecimal("5.00"), LocalDate.of(2025, 2, 3), false, "drink"),
        Transaction("Gorivo marec", BigDecimal("60.00"), LocalDate.of(2025, 3, 20), false, "transport"),
    )

    // javni readonly pogled, ki ga uporabljajo dashboard/analiza
    val transactions: List<Transaction>
        get() = _transactions

    /** Ročno doda transakcijo (na začetek, da je najnovejša na vrhu). */
    fun addTransaction(tx: Transaction) {
        _transactions.add(0, tx)
    }

    /**
     * Doda transakcijo iz bančne notifikacije.
     */
    fun addFromNotification(
        amount: BigDecimal,
        merchant: String,
        date: LocalDate,
        isIncome: Boolean,
        ...
        val tx = Transaction(
            name = merchant,
            amount = amount,          // znesek shranimo pozitiven, isIncome pove smer
            date = date,
            isIncome = isIncome,
            type = category
        )
